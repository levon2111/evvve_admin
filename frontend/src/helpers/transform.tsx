...
export const isDayJs = (v: any): boolean => {
  if (v instanceof Date || typeof v === 'string') {
    ...
    // Convert the input to ISO string and compare with the parsed date's ISO string
    const inputIsoString = (v instanceof Date ? v.toISOString() : v);
    return inputIsoString.includes(parsedDate.toISOString().replace('Z', ''));
  }
  return false;
};
